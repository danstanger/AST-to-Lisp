...
(defun jnil-generate-code (java-ast dialect verbose compiler-switches)
  (let ((*verbose-output* verbose)
	...
	(dolist (visitor visitors)
	  ...
	  (when *verbose-output* (stdout "~&;;; After running visitor: ~w~%" visitor)))
      (error "No coders found for the chosen dialect: ~a" dialect))
    (node-code java-ast)))

(definline lisp-output-pathname (qualified-unit-name dialect)
  (let ((split-qualified-name (uiop:split-string qualified-unit-name :separator "." qualified-unit-name)))
    (make-pathname
...
;;;;primitive types of the same name).
  ...
  (let ((name (lastelt (uiop:split-string qualified-name :separator "."))))
...
(defun collect-code (node-list)
 ; (break "jnil-utils 150 node-list ~a ~a ~a ~%" node-list (type-of node-list) (length node-list))
  (let ((li (mapcar #'(lambda (node) (node-code node)) node-list)))
(dolist (e li) (format t "type-of ~a ~a~%" e (type-of e))))
...
(trace java-to-lisp-type collect-code slots-and-others)
...
;;;;order to simplify the regular expressions.
  ...
  (re-let "(0\x([^lL]*))|(0([^lLfFdD\\.]+))|(([^dDfFlL]*)[dDfFlL]?)" token
	  ...
	  (values hex oct dec)))
|#
(defun parse-java-int-from-string (token) (values 0 0 0))
