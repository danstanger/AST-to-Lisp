...
(defcode linj-defnew (varlist ?documentation body)
 :format ("~:<defnew (~{~a~^ ~})~1I~:@_~@[~w~:@_~]~@[~w~]~:>"
          varlist documentation body))
#|
 (DEFCLASS LINJ-DEFNEW (CODE)
  ((VARLIST :INITARG :VARLIST :INITFORM NIL :ACCESSOR LINJ-DEFNEW-VARLIST)
   (DOCUMENTATION :INITARG :DOCUMENTATION :INITFORM NIL :ACCESSOR LINJ-DEFNEW-DOCUMENTATION)
   (BODY :INITARG :BODY :INITFORM NIL :ACCESSOR LINJ-DEFNEW-BODY)))
 (DEFUN MAKE-LINJ-DEFNEW (VARLIST BODY)
  (APPLY #'MAKE-INSTANCE (APPEND (LIST 'LINJ-DEFNEW) (LIST :VARLIST VARLIST) (LIST :BODY BODY))))
 (DEFMETHOD PRINT-OBJECT ((G25946 LINJ-DEFNEW) G25947)
  (WITH-SLOTS (VARLIST DOCUMENTATION BODY) G25946
    (format t "~&defnew args varlist ~a documentation ~a body ~a~%" varlist documentation body)
   (FORMAT G25947 "~:<defnew (~{~a~^ ~})~1I~:@_~>" (LIST VARLIST DOCUMENTATION BODY))))
|#
