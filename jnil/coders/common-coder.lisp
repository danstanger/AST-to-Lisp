...
;(break "safe-initializer-cl ~a" node)
;(break "safe-initializer-cld ~a" (declaration-initializer node))
;(break "safe-initializer-cln ~a" (node-code (declaration-initializer node)))
...
;  (break "before let ~a" node)
;  (break "before d ~a" (declaration-name node))
;  (break "before s ~a" (safe-initializer-cl node))
;  (break "before f ~a" (final-p node))
  (let ((name (make-lisp-name (java-to-lisp-name (declaration-name node) (final-p node))))
        (initializer (safe-initializer-cl node)))

;  (break "common coder after let ~a" node)
    (cond ((not (field-p node))
;(break "not fieldp  ~a~%" (field-p node))
           (make-lisp-sexp name initializer))
          ((static-p node) 
;(break "staticp  ~a~%" (static-p node))
...
;(break "before slot code return ~a~%" slot-code)
               slot-code)))))
(trace make-lisp-sexp)
                        ...
                        (append (collect-code superinterfaces)
                                (mklist (aif superclass (declaration-superclass node)
                                          ...
                                          (when (and superinterfaces (not (interface-p node))) 
                                            "object"))))
; It appears that the slots which are lisp classes in the list are being
; treated as lists. Not sure how you can push a function onto a class, but
; maybe I misunderstood the code.
; I will use the same crude hack as I did for other-decls below and refactor
; later
                        (push-readers-and-writers (let* ((a1 (collect-code slots))
							 (a1l (read-from-string (format nil "~a" a1))))
						    a1l)
                                                  getters nil) ; removed 'setters' (see bug 1)
; Not sure why the first collect code is not producing a list of lists,
; but the crude hack of writing and reading it will get me past this
; problem for now.
			(let* ((a1 (collect-code other-decls))
			       (a1l (read-from-string (format nil "~a" a1)))
                               (a2 (collect-code (declaration-types node)))
                               (a3 (collect-code others)))
			  (format t "a1 ~a~%" a1)
			  (format t "apply ~a~%" (apply #'append a1l))
			  (format t "type-of apply ~a~%" (type-of (apply #'append a1l)))
			  (format t "a2 ~a~%" a2)
			  (format t "a3 ~a~%" a3)
                          (append (apply #'append a1l)
                                  a2
                                  a3)))))
