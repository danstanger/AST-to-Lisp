...
(def-java-method (declaration-binding "resolveBinding2") (variable-declaration))
...
(defmethod final-p ((decl variable-declaration))
 ; (break "final-p ~a" decl) 
 ; (break "final-pt ~a" (type-of decl)) 
 ; (break "final-pd ~a" (declaration-binding decl)) 
 ; (break "final-pm ~a" (binding-modifiers (declaration-binding decl))) 
...
(defmethod type-name ((decl variable-declaration))
  (format t "var dec dec; ~a~%" decl)
  (format t "var dec decl declaration-binding ~a~%" (declaration-binding decl))
  (format t "var dec decl declaration-binding slots ~a~%"
	   (class-of (declaration-binding decl)))
  (format t "var dec decl binding-type-binding ~a~%" (binding-type-binding (declaration-binding decl))) ;this should be a binding, not a simple-type
  ;(let ((c (binding-type-binding (declaration-binding decl))))
   ; (format t "var dec type binding ~a~%" (type-binding c)))
